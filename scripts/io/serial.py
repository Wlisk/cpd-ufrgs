...
class Serial(IOBase):
    ...
    # reads the last item on an entity file
    def read_last(self) -> AllEType:
        end_pos = self._file.seek(0, SEEK_END)
        header_pos = self._file.seek(self._header.struct_size)
        if end_pos <= header_pos: return None
        end_pos -= self._entity.struct_size 
        data = super().read_at(end_pos)
        return self._classtype.make(\
            unpack(self._entity.struct_format, data) \
        ).normalize()
